...
public class DataListRealmProxy extends DataList
    implements RealmObjectProxy {

    static final class DataListColumnInfo extends ColumnInfo {
        ...
        DataListColumnInfo(String path, Table table) {
            final Map<String, Long> indicesMap = new HashMap<String, Long>(30);
            this.docIdIndex = getValidColumnIndex(path, table, "DataList", "docId");
            indicesMap.put("docId", this.docIdIndex);

            this.urlIndex = getValidColumnIndex(path, table, "DataList", "url");
            indicesMap.put("url", this.urlIndex);

            this.siteNameIndex = getValidColumnIndex(path, table, "DataList", "siteName");
            indicesMap.put("siteName", this.siteNameIndex);

            this.channelNameIndex = getValidColumnIndex(path, table, "DataList", "channelName");
            indicesMap.put("channelName", this.channelNameIndex);

            this.mediaTypeIndex = getValidColumnIndex(path, table, "DataList", "mediaType");
            indicesMap.put("mediaType", this.mediaTypeIndex);

            this.titleIndex = getValidColumnIndex(path, table, "DataList", "title");
            indicesMap.put("title", this.titleIndex);

            this.contentIndex = getValidColumnIndex(path, table, "DataList", "content");
            indicesMap.put("content", this.contentIndex);

            this.publishDateIndex = getValidColumnIndex(path, table, "DataList", "publishDate");
            indicesMap.put("publishDate", this.publishDateIndex);

            this.authorIndex = getValidColumnIndex(path, table, "DataList", "author");
            indicesMap.put("author", this.authorIndex);

            this.replyNumIndex = getValidColumnIndex(path, table, "DataList", "replyNum");
            indicesMap.put("replyNum", this.replyNumIndex);

            this.clickNumIndex = getValidColumnIndex(path, table, "DataList", "clickNum");
            indicesMap.put("clickNum", this.clickNumIndex);

            this.copyFromIndex = getValidColumnIndex(path, table, "DataList", "copyFrom");
            indicesMap.put("copyFrom", this.copyFromIndex);

            this.isAbroadIndex = getValidColumnIndex(path, table, "DataList", "isAbroad");
            indicesMap.put("isAbroad", this.isAbroadIndex);

            this.isOverseaIndex = getValidColumnIndex(path, table, "DataList", "isOversea");
            indicesMap.put("isOversea", this.isOverseaIndex);

            this.imageUrlsIndex = getValidColumnIndex(path, table, "DataList", "imageUrls");
            indicesMap.put("imageUrls", this.imageUrlsIndex);

            this.summaryIndex = getValidColumnIndex(path, table, "DataList", "summary");
            indicesMap.put("summary", this.summaryIndex);

            this.negativeStatusIndex = getValidColumnIndex(path, table, "DataList", "negativeStatus");
            indicesMap.put("negativeStatus", this.negativeStatusIndex);

            this.repostNumIndex = getValidColumnIndex(path, table, "DataList", "repostNum");
            indicesMap.put("repostNum", this.repostNumIndex);

            this.profileUrlIndex = getValidColumnIndex(path, table, "DataList", "profileUrl");
            indicesMap.put("profileUrl", this.profileUrlIndex);

            this.nickNameIndex = getValidColumnIndex(path, table, "DataList", "nickName");
            indicesMap.put("nickName", this.nickNameIndex);

            this.providerIdIndex = getValidColumnIndex(path, table, "DataList", "providerId");
            indicesMap.put("providerId", this.providerIdIndex);

            this.srcNickNameIndex = getValidColumnIndex(path, table, "DataList", "srcNickName");
            indicesMap.put("srcNickName", this.srcNickNameIndex);

            this.srcContentIndex = getValidColumnIndex(path, table, "DataList", "srcContent");
            indicesMap.put("srcContent", this.srcContentIndex);

            this.srcPublishDateIndex = getValidColumnIndex(path, table, "DataList", "srcPublishDate");
            indicesMap.put("srcPublishDate", this.srcPublishDateIndex);

            this.srcRepostNumIndex = getValidColumnIndex(path, table, "DataList", "srcRepostNum");
            indicesMap.put("srcRepostNum", this.srcRepostNumIndex);

            this.srcCommentNumIndex = getValidColumnIndex(path, table, "DataList", "srcCommentNum");
            indicesMap.put("srcCommentNum", this.srcCommentNumIndex);

            this.srcPicUrlsIndex = getValidColumnIndex(path, table, "DataList", "srcPicUrls");
            indicesMap.put("srcPicUrls", this.srcPicUrlsIndex);

            this.srcPicPathsIndex = getValidColumnIndex(path, table, "DataList", "srcPicPaths");
            indicesMap.put("srcPicPaths", this.srcPicPathsIndex);

            this.keywordsIndex = getValidColumnIndex(path, table, "DataList", "keywords");
            indicesMap.put("keywords", this.keywordsIndex);

            this.logoIndex = getValidColumnIndex(path, table, "DataList", "logo");
    ...
    private final DataListColumnInfo columnInfo;
    ...
    DataListRealmProxy(ColumnInfo columnInfo) {
        this.columnInfo = (DataListColumnInfo) columnInfo;
    }
    ...
    public static Table initTable(ImplicitTransaction transaction) {
        if (!transaction.hasTable("class_DataList")) {
            Table table = transaction.getTable("class_DataList");
        ...
        return transaction.getTable("class_DataList");
    }

    public static DataListColumnInfo validateTable(ImplicitTransaction transaction) {
        if (transaction.hasTable("class_DataList")) {
            Table table = transaction.getTable("class_DataList");
            ...
            final DataListColumnInfo columnInfo = new DataListColumnInfo(transaction.getPath(), table);
            ...
            throw new RealmMigrationNeededException(transaction.getPath(), "The DataList class is missing from the schema for this Realm.");
        }
    }

    public static String getTableName() {
        return "class_DataList";
    }
    ...
    @SuppressWarnings("cast")
    public static DataList createOrUpdateUsingJsonObject(Realm realm, JSONObject json, boolean update)
        throws JSONException {
        DataList obj = realm.createObject(DataList.class);
        if (json.has("docId")) {
    ...
    @SuppressWarnings("cast")
    public static DataList createUsingJsonStream(Realm realm, JsonReader reader)
        throws IOException {
        DataList obj = realm.createObject(DataList.class);
        reader.beginObject();
    ...
    public static DataList copyOrUpdate(Realm realm, DataList object, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
    ...
    public static DataList copy(Realm realm, DataList newObject, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
        DataList realmObject = realm.createObject(DataList.class);
    ...
    public static DataList createDetachedCopy(DataList realmObject, int currentDepth, int maxDepth, Map<RealmObject, CacheData<RealmObject>> cache) {
        if (currentDepth > maxDepth || realmObject == null) {
            return null;
        }
        CacheData<DataList> cachedObject = (CacheData) cache.get(realmObject);
        DataList standaloneObject;
    ...
    public String toString() {
        ...
        StringBuilder stringBuilder = new StringBuilder("DataList = [");
    ...
    public boolean equals(Object o) {
        ...
        DataListRealmProxy aDataList = (DataListRealmProxy)o;

        String path = realm.getPath();
        String otherPath = aDataList.realm.getPath();
        if (path != null ? !path.equals(otherPath) : otherPath != null) return false;;

        String tableName = row.getTable().getName();
        String otherTableName = aDataList.row.getTable().getName();
        if (tableName != null ? !tableName.equals(otherTableName) : otherTableName != null) return false;

        if (row.getIndex() != aDataList.row.getIndex()) return false;
