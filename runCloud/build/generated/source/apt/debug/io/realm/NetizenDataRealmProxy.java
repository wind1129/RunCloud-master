...
public class NetizenDataRealmProxy extends NetizenData
    implements RealmObjectProxy {

    static final class NetizenDataColumnInfo extends ColumnInfo {
        ...
        NetizenDataColumnInfo(String path, Table table) {
            final Map<String, Long> indicesMap = new HashMap<String, Long>(29);
            this.docIdIndex = getValidColumnIndex(path, table, "NetizenData", "docId");
            indicesMap.put("docId", this.docIdIndex);

            this.urlIndex = getValidColumnIndex(path, table, "NetizenData", "url");
            indicesMap.put("url", this.urlIndex);

            this.siteNameIndex = getValidColumnIndex(path, table, "NetizenData", "siteName");
            indicesMap.put("siteName", this.siteNameIndex);

            this.channelNameIndex = getValidColumnIndex(path, table, "NetizenData", "channelName");
            indicesMap.put("channelName", this.channelNameIndex);

            this.mediaTypeIndex = getValidColumnIndex(path, table, "NetizenData", "mediaType");
            indicesMap.put("mediaType", this.mediaTypeIndex);

            this.titleIndex = getValidColumnIndex(path, table, "NetizenData", "title");
            indicesMap.put("title", this.titleIndex);

            this.contentIndex = getValidColumnIndex(path, table, "NetizenData", "content");
            indicesMap.put("content", this.contentIndex);

            this.publishDateIndex = getValidColumnIndex(path, table, "NetizenData", "publishDate");
            indicesMap.put("publishDate", this.publishDateIndex);

            this.authorIndex = getValidColumnIndex(path, table, "NetizenData", "author");
            indicesMap.put("author", this.authorIndex);

            this.replyNumIndex = getValidColumnIndex(path, table, "NetizenData", "replyNum");
            indicesMap.put("replyNum", this.replyNumIndex);

            this.clickNumIndex = getValidColumnIndex(path, table, "NetizenData", "clickNum");
            indicesMap.put("clickNum", this.clickNumIndex);

            this.copyFromIndex = getValidColumnIndex(path, table, "NetizenData", "copyFrom");
            indicesMap.put("copyFrom", this.copyFromIndex);

            this.isAbroadIndex = getValidColumnIndex(path, table, "NetizenData", "isAbroad");
            indicesMap.put("isAbroad", this.isAbroadIndex);

            this.isOverseaIndex = getValidColumnIndex(path, table, "NetizenData", "isOversea");
            indicesMap.put("isOversea", this.isOverseaIndex);

            this.imageUrlsIndex = getValidColumnIndex(path, table, "NetizenData", "imageUrls");
            indicesMap.put("imageUrls", this.imageUrlsIndex);

            this.summaryIndex = getValidColumnIndex(path, table, "NetizenData", "summary");
            indicesMap.put("summary", this.summaryIndex);

            this.negativeStatusIndex = getValidColumnIndex(path, table, "NetizenData", "negativeStatus");
            indicesMap.put("negativeStatus", this.negativeStatusIndex);

            this.repostNumIndex = getValidColumnIndex(path, table, "NetizenData", "repostNum");
            indicesMap.put("repostNum", this.repostNumIndex);

            this.profileUrlIndex = getValidColumnIndex(path, table, "NetizenData", "profileUrl");
            indicesMap.put("profileUrl", this.profileUrlIndex);

            this.nickNameIndex = getValidColumnIndex(path, table, "NetizenData", "nickName");
            indicesMap.put("nickName", this.nickNameIndex);

            this.providerIdIndex = getValidColumnIndex(path, table, "NetizenData", "providerId");
            indicesMap.put("providerId", this.providerIdIndex);

            this.srcNickNameIndex = getValidColumnIndex(path, table, "NetizenData", "srcNickName");
            indicesMap.put("srcNickName", this.srcNickNameIndex);

            this.srcContentIndex = getValidColumnIndex(path, table, "NetizenData", "srcContent");
            indicesMap.put("srcContent", this.srcContentIndex);

            this.srcPublishDateIndex = getValidColumnIndex(path, table, "NetizenData", "srcPublishDate");
            indicesMap.put("srcPublishDate", this.srcPublishDateIndex);

            this.srcRepostNumIndex = getValidColumnIndex(path, table, "NetizenData", "srcRepostNum");
            indicesMap.put("srcRepostNum", this.srcRepostNumIndex);

            this.srcCommentNumIndex = getValidColumnIndex(path, table, "NetizenData", "srcCommentNum");
            indicesMap.put("srcCommentNum", this.srcCommentNumIndex);

            this.srcPicUrlsIndex = getValidColumnIndex(path, table, "NetizenData", "srcPicUrls");
            indicesMap.put("srcPicUrls", this.srcPicUrlsIndex);

            this.srcPicPathsIndex = getValidColumnIndex(path, table, "NetizenData", "srcPicPaths");
            indicesMap.put("srcPicPaths", this.srcPicPathsIndex);

            this.logoIndex = getValidColumnIndex(path, table, "NetizenData", "logo");
    ...
    private final NetizenDataColumnInfo columnInfo;
    ...
    NetizenDataRealmProxy(ColumnInfo columnInfo) {
        this.columnInfo = (NetizenDataColumnInfo) columnInfo;
    }
    ...
    public static Table initTable(ImplicitTransaction transaction) {
        if (!transaction.hasTable("class_NetizenData")) {
            Table table = transaction.getTable("class_NetizenData");
        ...
        return transaction.getTable("class_NetizenData");
    }

    public static NetizenDataColumnInfo validateTable(ImplicitTransaction transaction) {
        if (transaction.hasTable("class_NetizenData")) {
            Table table = transaction.getTable("class_NetizenData");
            ...
            final NetizenDataColumnInfo columnInfo = new NetizenDataColumnInfo(transaction.getPath(), table);
            ...
            throw new RealmMigrationNeededException(transaction.getPath(), "The NetizenData class is missing from the schema for this Realm.");
        }
    }

    public static String getTableName() {
        return "class_NetizenData";
    }
    ...
    @SuppressWarnings("cast")
    public static NetizenData createOrUpdateUsingJsonObject(Realm realm, JSONObject json, boolean update)
        throws JSONException {
        NetizenData obj = realm.createObject(NetizenData.class);
        if (json.has("docId")) {
    ...
    @SuppressWarnings("cast")
    public static NetizenData createUsingJsonStream(Realm realm, JsonReader reader)
        throws IOException {
        NetizenData obj = realm.createObject(NetizenData.class);
        reader.beginObject();
    ...
    public static NetizenData copyOrUpdate(Realm realm, NetizenData object, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
    ...
    public static NetizenData copy(Realm realm, NetizenData newObject, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
        NetizenData realmObject = realm.createObject(NetizenData.class);
    ...
    public static NetizenData createDetachedCopy(NetizenData realmObject, int currentDepth, int maxDepth, Map<RealmObject, CacheData<RealmObject>> cache) {
        if (currentDepth > maxDepth || realmObject == null) {
            return null;
        }
        CacheData<NetizenData> cachedObject = (CacheData) cache.get(realmObject);
        NetizenData standaloneObject;
        if (cachedObject != null) {
            ...
            standaloneObject = new NetizenData();
    ...
    public String toString() {
        ...
        StringBuilder stringBuilder = new StringBuilder("NetizenData = [");
    ...
    public boolean equals(Object o) {
        ...
        NetizenDataRealmProxy aNetizenData = (NetizenDataRealmProxy)o;

        String path = realm.getPath();
        String otherPath = aNetizenData.realm.getPath();
        if (path != null ? !path.equals(otherPath) : otherPath != null) return false;;

        String tableName = row.getTable().getName();
        String otherTableName = aNetizenData.row.getTable().getName();
        if (tableName != null ? !tableName.equals(otherTableName) : otherTableName != null) return false;

        if (row.getIndex() != aNetizenData.row.getIndex()) return false;
