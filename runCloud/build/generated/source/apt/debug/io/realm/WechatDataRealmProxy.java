...
public class WechatDataRealmProxy extends WechatData
    implements RealmObjectProxy {

    static final class WechatDataColumnInfo extends ColumnInfo {
        ...
        WechatDataColumnInfo(String path, Table table) {
            final Map<String, Long> indicesMap = new HashMap<String, Long>(29);
            this.docIdIndex = getValidColumnIndex(path, table, "WechatData", "docId");
            indicesMap.put("docId", this.docIdIndex);

            this.urlIndex = getValidColumnIndex(path, table, "WechatData", "url");
            indicesMap.put("url", this.urlIndex);

            this.siteNameIndex = getValidColumnIndex(path, table, "WechatData", "siteName");
            indicesMap.put("siteName", this.siteNameIndex);

            this.channelNameIndex = getValidColumnIndex(path, table, "WechatData", "channelName");
            indicesMap.put("channelName", this.channelNameIndex);

            this.mediaTypeIndex = getValidColumnIndex(path, table, "WechatData", "mediaType");
            indicesMap.put("mediaType", this.mediaTypeIndex);

            this.titleIndex = getValidColumnIndex(path, table, "WechatData", "title");
            indicesMap.put("title", this.titleIndex);

            this.contentIndex = getValidColumnIndex(path, table, "WechatData", "content");
            indicesMap.put("content", this.contentIndex);

            this.publishDateIndex = getValidColumnIndex(path, table, "WechatData", "publishDate");
            indicesMap.put("publishDate", this.publishDateIndex);

            this.authorIndex = getValidColumnIndex(path, table, "WechatData", "author");
            indicesMap.put("author", this.authorIndex);

            this.replyNumIndex = getValidColumnIndex(path, table, "WechatData", "replyNum");
            indicesMap.put("replyNum", this.replyNumIndex);

            this.clickNumIndex = getValidColumnIndex(path, table, "WechatData", "clickNum");
            indicesMap.put("clickNum", this.clickNumIndex);

            this.copyFromIndex = getValidColumnIndex(path, table, "WechatData", "copyFrom");
            indicesMap.put("copyFrom", this.copyFromIndex);

            this.isAbroadIndex = getValidColumnIndex(path, table, "WechatData", "isAbroad");
            indicesMap.put("isAbroad", this.isAbroadIndex);

            this.isOverseaIndex = getValidColumnIndex(path, table, "WechatData", "isOversea");
            indicesMap.put("isOversea", this.isOverseaIndex);

            this.imageUrlsIndex = getValidColumnIndex(path, table, "WechatData", "imageUrls");
            indicesMap.put("imageUrls", this.imageUrlsIndex);

            this.summaryIndex = getValidColumnIndex(path, table, "WechatData", "summary");
            indicesMap.put("summary", this.summaryIndex);

            this.negativeStatusIndex = getValidColumnIndex(path, table, "WechatData", "negativeStatus");
            indicesMap.put("negativeStatus", this.negativeStatusIndex);

            this.repostNumIndex = getValidColumnIndex(path, table, "WechatData", "repostNum");
            indicesMap.put("repostNum", this.repostNumIndex);

            this.profileUrlIndex = getValidColumnIndex(path, table, "WechatData", "profileUrl");
            indicesMap.put("profileUrl", this.profileUrlIndex);

            this.nickNameIndex = getValidColumnIndex(path, table, "WechatData", "nickName");
            indicesMap.put("nickName", this.nickNameIndex);

            this.providerIdIndex = getValidColumnIndex(path, table, "WechatData", "providerId");
            indicesMap.put("providerId", this.providerIdIndex);

            this.srcNickNameIndex = getValidColumnIndex(path, table, "WechatData", "srcNickName");
            indicesMap.put("srcNickName", this.srcNickNameIndex);

            this.srcContentIndex = getValidColumnIndex(path, table, "WechatData", "srcContent");
            indicesMap.put("srcContent", this.srcContentIndex);

            this.srcPublishDateIndex = getValidColumnIndex(path, table, "WechatData", "srcPublishDate");
            indicesMap.put("srcPublishDate", this.srcPublishDateIndex);

            this.srcRepostNumIndex = getValidColumnIndex(path, table, "WechatData", "srcRepostNum");
            indicesMap.put("srcRepostNum", this.srcRepostNumIndex);

            this.srcCommentNumIndex = getValidColumnIndex(path, table, "WechatData", "srcCommentNum");
            indicesMap.put("srcCommentNum", this.srcCommentNumIndex);

            this.srcPicUrlsIndex = getValidColumnIndex(path, table, "WechatData", "srcPicUrls");
            indicesMap.put("srcPicUrls", this.srcPicUrlsIndex);

            this.srcPicPathsIndex = getValidColumnIndex(path, table, "WechatData", "srcPicPaths");
            indicesMap.put("srcPicPaths", this.srcPicPathsIndex);

            this.logoIndex = getValidColumnIndex(path, table, "WechatData", "logo");
    ...
    private final WechatDataColumnInfo columnInfo;
    ...
    WechatDataRealmProxy(ColumnInfo columnInfo) {
        this.columnInfo = (WechatDataColumnInfo) columnInfo;
    }
    ...
    public static Table initTable(ImplicitTransaction transaction) {
        if (!transaction.hasTable("class_WechatData")) {
            Table table = transaction.getTable("class_WechatData");
        ...
        return transaction.getTable("class_WechatData");
    }

    public static WechatDataColumnInfo validateTable(ImplicitTransaction transaction) {
        if (transaction.hasTable("class_WechatData")) {
            Table table = transaction.getTable("class_WechatData");
            ...
            final WechatDataColumnInfo columnInfo = new WechatDataColumnInfo(transaction.getPath(), table);
            ...
            throw new RealmMigrationNeededException(transaction.getPath(), "The WechatData class is missing from the schema for this Realm.");
        }
    }

    public static String getTableName() {
        return "class_WechatData";
    }
    ...
    @SuppressWarnings("cast")
    public static WechatData createOrUpdateUsingJsonObject(Realm realm, JSONObject json, boolean update)
        throws JSONException {
        WechatData obj = realm.createObject(WechatData.class);
        if (json.has("docId")) {
    ...
    @SuppressWarnings("cast")
    public static WechatData createUsingJsonStream(Realm realm, JsonReader reader)
        throws IOException {
        WechatData obj = realm.createObject(WechatData.class);
        reader.beginObject();
    ...
    public static WechatData copyOrUpdate(Realm realm, WechatData object, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
    ...
    public static WechatData copy(Realm realm, WechatData newObject, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
        WechatData realmObject = realm.createObject(WechatData.class);
    ...
    public static WechatData createDetachedCopy(WechatData realmObject, int currentDepth, int maxDepth, Map<RealmObject, CacheData<RealmObject>> cache) {
        if (currentDepth > maxDepth || realmObject == null) {
            return null;
        }
        CacheData<WechatData> cachedObject = (CacheData) cache.get(realmObject);
        WechatData standaloneObject;
        if (cachedObject != null) {
            ...
            standaloneObject = new WechatData();
    ...
    public String toString() {
        ...
        StringBuilder stringBuilder = new StringBuilder("WechatData = [");
    ...
    public boolean equals(Object o) {
        ...
        WechatDataRealmProxy aWechatData = (WechatDataRealmProxy)o;

        String path = realm.getPath();
        String otherPath = aWechatData.realm.getPath();
        if (path != null ? !path.equals(otherPath) : otherPath != null) return false;;

        String tableName = row.getTable().getName();
        String otherTableName = aWechatData.row.getTable().getName();
        if (tableName != null ? !tableName.equals(otherTableName) : otherTableName != null) return false;

        if (row.getIndex() != aWechatData.row.getIndex()) return false;
