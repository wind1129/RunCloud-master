...
public class SearchDataRealmProxy extends SearchData
    implements RealmObjectProxy {

    static final class SearchDataColumnInfo extends ColumnInfo {
        ...
        SearchDataColumnInfo(String path, Table table) {
            final Map<String, Long> indicesMap = new HashMap<String, Long>(30);
            this.docIdIndex = getValidColumnIndex(path, table, "SearchData", "docId");
            indicesMap.put("docId", this.docIdIndex);

            this.urlIndex = getValidColumnIndex(path, table, "SearchData", "url");
            indicesMap.put("url", this.urlIndex);

            this.siteNameIndex = getValidColumnIndex(path, table, "SearchData", "siteName");
            indicesMap.put("siteName", this.siteNameIndex);

            this.channelNameIndex = getValidColumnIndex(path, table, "SearchData", "channelName");
            indicesMap.put("channelName", this.channelNameIndex);

            this.mediaTypeIndex = getValidColumnIndex(path, table, "SearchData", "mediaType");
            indicesMap.put("mediaType", this.mediaTypeIndex);

            this.titleIndex = getValidColumnIndex(path, table, "SearchData", "title");
            indicesMap.put("title", this.titleIndex);

            this.contentIndex = getValidColumnIndex(path, table, "SearchData", "content");
            indicesMap.put("content", this.contentIndex);

            this.publishDateIndex = getValidColumnIndex(path, table, "SearchData", "publishDate");
            indicesMap.put("publishDate", this.publishDateIndex);

            this.authorIndex = getValidColumnIndex(path, table, "SearchData", "author");
            indicesMap.put("author", this.authorIndex);

            this.replyNumIndex = getValidColumnIndex(path, table, "SearchData", "replyNum");
            indicesMap.put("replyNum", this.replyNumIndex);

            this.clickNumIndex = getValidColumnIndex(path, table, "SearchData", "clickNum");
            indicesMap.put("clickNum", this.clickNumIndex);

            this.copyFromIndex = getValidColumnIndex(path, table, "SearchData", "copyFrom");
            indicesMap.put("copyFrom", this.copyFromIndex);

            this.isAbroadIndex = getValidColumnIndex(path, table, "SearchData", "isAbroad");
            indicesMap.put("isAbroad", this.isAbroadIndex);

            this.isOverseaIndex = getValidColumnIndex(path, table, "SearchData", "isOversea");
            indicesMap.put("isOversea", this.isOverseaIndex);

            this.imageUrlsIndex = getValidColumnIndex(path, table, "SearchData", "imageUrls");
            indicesMap.put("imageUrls", this.imageUrlsIndex);

            this.summaryIndex = getValidColumnIndex(path, table, "SearchData", "summary");
            indicesMap.put("summary", this.summaryIndex);

            this.negativeStatusIndex = getValidColumnIndex(path, table, "SearchData", "negativeStatus");
            indicesMap.put("negativeStatus", this.negativeStatusIndex);

            this.repostNumIndex = getValidColumnIndex(path, table, "SearchData", "repostNum");
            indicesMap.put("repostNum", this.repostNumIndex);

            this.profileUrlIndex = getValidColumnIndex(path, table, "SearchData", "profileUrl");
            indicesMap.put("profileUrl", this.profileUrlIndex);

            this.nickNameIndex = getValidColumnIndex(path, table, "SearchData", "nickName");
            indicesMap.put("nickName", this.nickNameIndex);

            this.providerIdIndex = getValidColumnIndex(path, table, "SearchData", "providerId");
            indicesMap.put("providerId", this.providerIdIndex);

            this.srcNickNameIndex = getValidColumnIndex(path, table, "SearchData", "srcNickName");
            indicesMap.put("srcNickName", this.srcNickNameIndex);

            this.srcContentIndex = getValidColumnIndex(path, table, "SearchData", "srcContent");
            indicesMap.put("srcContent", this.srcContentIndex);

            this.srcPublishDateIndex = getValidColumnIndex(path, table, "SearchData", "srcPublishDate");
            indicesMap.put("srcPublishDate", this.srcPublishDateIndex);

            this.srcRepostNumIndex = getValidColumnIndex(path, table, "SearchData", "srcRepostNum");
            indicesMap.put("srcRepostNum", this.srcRepostNumIndex);

            this.srcCommentNumIndex = getValidColumnIndex(path, table, "SearchData", "srcCommentNum");
            indicesMap.put("srcCommentNum", this.srcCommentNumIndex);

            this.srcPicUrlsIndex = getValidColumnIndex(path, table, "SearchData", "srcPicUrls");
            indicesMap.put("srcPicUrls", this.srcPicUrlsIndex);

            this.srcPicPathsIndex = getValidColumnIndex(path, table, "SearchData", "srcPicPaths");
            indicesMap.put("srcPicPaths", this.srcPicPathsIndex);

            this.keywordsIndex = getValidColumnIndex(path, table, "SearchData", "keywords");
            indicesMap.put("keywords", this.keywordsIndex);

            this.logoIndex = getValidColumnIndex(path, table, "SearchData", "logo");
    ...
    private final SearchDataColumnInfo columnInfo;
    ...
    SearchDataRealmProxy(ColumnInfo columnInfo) {
        this.columnInfo = (SearchDataColumnInfo) columnInfo;
    }
    ...
    public String getKeywords() {
        realm.checkIfValid();
        return (java.lang.String) row.getString(columnInfo.keywordsIndex);
    }
    ...
    public void setKeywords(String value) {
        realm.checkIfValid();
        if (value == null) {
            row.setNull(columnInfo.keywordsIndex);
            return;
        }
        row.setString(columnInfo.keywordsIndex, value);
    }
    ...
    public static Table initTable(ImplicitTransaction transaction) {
        if (!transaction.hasTable("class_SearchData")) {
            Table table = transaction.getTable("class_SearchData");
            ...
            table.addColumn(RealmFieldType.STRING, "keywords", Table.NULLABLE);
            table.addColumn(RealmFieldType.STRING, "logo", Table.NULLABLE);
            table.setPrimaryKey("");
            return table;
        }
        return transaction.getTable("class_SearchData");
    }

    public static SearchDataColumnInfo validateTable(ImplicitTransaction transaction) {
        if (transaction.hasTable("class_SearchData")) {
            Table table = transaction.getTable("class_SearchData");
            ...
            final SearchDataColumnInfo columnInfo = new SearchDataColumnInfo(transaction.getPath(), table);
            ...
            if (!columnTypes.containsKey("keywords")) {
                throw new RealmMigrationNeededException(transaction.getPath(), "Missing field 'keywords' in existing Realm file. Either remove field or migrate using io.realm.internal.Table.addColumn().");
            }
            if (columnTypes.get("keywords") != RealmFieldType.STRING) {
                throw new RealmMigrationNeededException(transaction.getPath(), "Invalid type 'String' for field 'keywords' in existing Realm file.");
            }
            if (!table.isColumnNullable(columnInfo.keywordsIndex)) {
                throw new RealmMigrationNeededException(transaction.getPath(), "Field 'keywords' is required. Either set @Required to field 'keywords' or migrate using io.realm.internal.Table.convertColumnToNullable().");
            }
            ...
            throw new RealmMigrationNeededException(transaction.getPath(), "The SearchData class is missing from the schema for this Realm.");
        }
    }
    ...
    @SuppressWarnings("cast")
    public static SearchData createOrUpdateUsingJsonObject(Realm realm, JSONObject json, boolean update)
        throws JSONException {
        SearchData obj = realm.createObject(SearchData.class);
        if (json.has("docId")) {
        ...
        if (json.has("keywords")) {
            if (json.isNull("keywords")) {
                obj.setKeywords(null);
            } else {
                obj.setKeywords((String) json.getString("keywords"));
            }
        }
        if (json.has("logo")) {
            if (json.isNull("logo")) {
    ...
    @SuppressWarnings("cast")
    public static SearchData createUsingJsonStream(Realm realm, JsonReader reader)
        throws IOException {
        SearchData obj = realm.createObject(SearchData.class);
        reader.beginObject();
        while (reader.hasNext()) {
            ...
            if (name.equals("docId")) {
                ...
                if (reader.peek() == JsonToken.NULL) {
                    ...
                    obj.setKeywords((String) reader.nextString());
                }
    ...
    public static SearchData copyOrUpdate(Realm realm, SearchData object, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
    ...
    public static SearchData copy(Realm realm, SearchData newObject, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
        SearchData realmObject = realm.createObject(SearchData.class);
        ...
        realmObject.setKeywords(newObject.getKeywords());
        realmObject.setLogo(newObject.getLogo());
        return realmObject;
    }

    public static SearchData createDetachedCopy(SearchData realmObject, int currentDepth, int maxDepth, Map<RealmObject, CacheData<RealmObject>> cache) {
        if (currentDepth > maxDepth || realmObject == null) {
            return null;
        }
        CacheData<SearchData> cachedObject = (CacheData) cache.get(realmObject);
        SearchData standaloneObject;
        if (cachedObject != null) {
            ...
            standaloneObject = new SearchData();
        ...
        standaloneObject.setKeywords(realmObject.getKeywords());
    ...
    public String toString() {
        ...
        StringBuilder stringBuilder = new StringBuilder("SearchData = [");
        ...
        stringBuilder.append(getKeywords() != null ? getKeywords() : "null");
        stringBuilder.append("}");
        stringBuilder.append(",");
    ...
    public boolean equals(Object o) {
        ...
        SearchDataRealmProxy aSearchData = (SearchDataRealmProxy)o;

        String path = realm.getPath();
        String otherPath = aSearchData.realm.getPath();
        if (path != null ? !path.equals(otherPath) : otherPath != null) return false;;

        String tableName = row.getTable().getName();
        String otherTableName = aSearchData.row.getTable().getName();
        if (tableName != null ? !tableName.equals(otherTableName) : otherTableName != null) return false;

        if (row.getIndex() != aSearchData.row.getIndex()) return false;
